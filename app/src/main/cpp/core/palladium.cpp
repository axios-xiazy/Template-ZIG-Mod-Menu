...
void main_thread() {
	sleep(2);
	//Wait load lib game and get base address no use sleep / usleep this use modern c++
	while(!(g_il2cppElf=ElfScanner::findElf("libil2cpp.so")).isValid())std::this_thread::sleep_for(100ms);
	uintptr_t il2cppBase = g_il2cppElf.base();	
	
	//Patch offset with kernel level non root ( built-in )
	StealthPatch::createWithHex(oxorany("libil2cpp.so"), oxorany(0x100000), oxorany("00 00 00 00")).Modify();
	
	//Patch offset with MemoryPatch ( backend kitty )
...
Java_zig_cheat_qq_native_1bridge_StealthJNI_Callback(JNIEnv *env, jclass clazz, jint id, jboolean check, jint value, jfloat value2, jstring value3) {
    switch (id) {
        case 1: config.Hackmap = (bool)check; break;
        case 2: config.ShowCoolDown = (bool)check; break;
        case 3: config.AimbotRange = (int)value; break;
        case 4: config.DrawLine = (bool)check; break;
        case 5: config.DrawBox = (bool)check; break;
        case 6: config.EspMode = (int)value; break;
        case 7: config.AntiReport = (bool)check; break;
        case 8: config.BypassIntegrity = (bool)check; break;
    }
}

}

__attribute__((constructor))void init(){std::thread(main_thread).detach();}