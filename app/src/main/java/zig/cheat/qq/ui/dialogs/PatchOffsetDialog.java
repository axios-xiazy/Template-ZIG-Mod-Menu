...
public class PatchOffsetDialog {
    ...
    private void createDialog() {
        ...
        freezeCheckBox.setTypeface(Utils.getFont(context));
    ...
    private EditText createTextInput(String hint, float d) {
        ...
        et.setHintTextColor(0xFF666666);
        et.setTextSize(10);
        et.setTypeface(Utils.getFont(context));
        et.setBackground(createInputBg(d));
    ...
    private void loadLibList() {
        executor.execute(() -> {
            try {
                ...
                String[] loaded = Jni.getLoadedLibNames();
                if (loaded != null) {
    ...
    private void readMemory() {
        ...
        executor.execute(() -> {
            try {
                ...
                long base = Jni.findLibBase(libName);
                if (base == 0) {
                ...
                String hexDump = Jni.readMemoryHexDump(addr, 64);
    ...
    private void scanPattern() {
        ...
        executor.execute(() -> {
            try {
                long[] results = Jni.scanPatternAll(sel.toString(), pattern, MAX_SCAN_RESULTS);
    ...
    private void addPatch() {
        ...
        executor.execute(() -> {
            try {
                ...
                int id = Jni.createPatch(libName, offset, hex, desc.isEmpty() ? "Patch" : desc);

                if (id >= 0) {
                    boolean applied = Jni.applyPatch(id);
    ...
    private void doExport(String dir) {
        executor.execute(() -> {
            try {
                String json = Jni.exportPatchesJson();
    ...
    private void doImport(String filePath) {
        executor.execute(() -> {
            try (FileReader reader = new FileReader(filePath)) {
                ...
                int count = Jni.importPatchesJson(sb.toString());
                handler.post(() -> {
                    refreshPatchList();
    ...
    private void refreshPatchList() {
        ...
        executor.execute(() -> {
            try {
                String[] patches = Jni.getPatchList();
                handler.post(() -> {
                    if (isDestroyed.get() || patchListLayout == null) return;
    ...
    private void addPatchCard(String[] parts) {
        ...
        title.setTextColor(active ? 0xFF10B981 : Menu.COLOR_TEXT_PRIMARY);
        ...
        details.setTextColor(Menu.COLOR_TEXT_SECONDARY);
